...
public class RailRoadLonely implements RailroadBarons {
    ...
    private Collection<model.Route> list_routes;
    private model.Route route;
    private java.util.Map<String, Station> stationMap;
    ...
    public void startAGameWith(RailroadMap map)
    {
        ...
        CompPlayer player2 = new CompPlayer(Baron.YELLOW,deck, this.getRailroadMap(), this);
        list_players.add(player2);
        CompPlayer player3 = new CompPlayer(Baron.GREEN,deck, this.getRailroadMap(), this);
        list_players.add(player3);
        CompPlayer player4 = new CompPlayer(Baron.RED,deck, this.getRailroadMap(), this);
    ...
    public void makeRouteGraph(){
        ...
        for(model.Route route: this.map.getRoutes()){
    ...
    public void buildedgeList(){
        ...
        for(model.Route route: this.getRailroadMap().getRoutes()){
    ...
    public boolean canCurrentPlayerClaimRoute(int row, int col) {
        for (model.Route route : list_routes)
        {
            try {
     ...
     *
     * @param row The row of a {@link Track} in the {@link model.Route} to claim.
    ...
    public boolean gameIsOver() {
        ArrayList<model.Route> list = new ArrayList<>();
        int count = 0;
        for (model.Route route : list_routes)
        {
        ...
        if (list.size() == 0)
        {
            for(Player player: list_players){
                for(CrossCountryRoute curcross: crosscountryroutes){
                    boolean ownall = true;
                    for(model.Route crossroutes: curcross.getRoutes()){
